...
# Añadimos la ruta raíz del proyecto al path para poder importar desde otros módulos
...
# ¡IMPORTANTE! Modifica estos valores con tu información real
...
def seed_data():
    ...
    try:
        ...
        if user:
            print(f"El usuario admin '{ADMIN_EMAIL}' ya existe. No se realizarán nuevas inserciones.")
            return

        print("Base de datos vacía. Sembrando datos iniciales...")

        # 2. Si no existe, crear el usuario admin con contraseña encriptada
        ...
        user_id = cursor.lastrowid # Obtenemos el ID del usuario recién creado (será 1)

        # 3. Añadir una lista de activos por defecto para este usuario
        print("Añadiendo activos por defecto (EUR/USD, GBP/USD, AUD/CAD)...")
        default_assets = ['EUR/USD', 'GBP/USD', 'BTC/USD']
        ...
        # Obtenemos el ID del primer activo para usarlo como el activo por defecto en la configuración
        cursor.execute("SELECT id FROM assets WHERE user_id = ? AND name = ?", (user_id, default_assets[0]))
        active_asset_id = cursor.fetchone()[0]

        # 4. Crear la fila de configuración por defecto asociada a este usuario
